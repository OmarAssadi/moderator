...
function COMMAND:OnRun(client, arguments, target)
    ...
    if (not moderator.groups[group]) then return moderator.Notify(client, "you provided an invalid usergroup.") end
...
if (SERVER) then
    concommand.Add("mod_setrank", function(client, command, arguments)
        if (not IsValid(client) or client:IsUserGroup("owner")) then
            if (not arguments[1]) then return MsgN("You did not enter a valid name.") end
