...
function PANEL:ToggleSideBar()
    local lastHidden = not self.sideHidden
    self.sideHidden = not self.sideHidden
...
function PANEL:Init()
    ...
    for k, v in SortedPairs(moderator.menus, true) do
        if (v.adminOnly and not LocalPlayer():CheckGroup(v.group or "moderator")) then continue end

        if (v.ShouldDisplay) then
            if (v:ShouldDisplay() == false) then continue end
        end
