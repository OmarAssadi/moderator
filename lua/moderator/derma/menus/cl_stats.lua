...
function CATEGORY:Layout(panel)
    ...
    timer.Create("mod_PingCheck", 0.75, 0, function()
        if (not IsValid(graph)) then return timer.Remove("mod_PingCheck") end
        local average = 0
        local pingList = {}
    ...
    timer.Create("mod_VolumeCheck", 0.75, 0, function()
        if (not IsValid(graph)) then return timer.Remove("mod_VolumeCheck") end
        local total = 0
    ...
    panel:AddHeader("Active Bans: " .. table.Count(moderator.bans), nil, function() return "Active Bans: " .. table.Count(moderator.bans) end)
    panel:AddHeader("Active Reports: " .. table.Count(moderator.reports or {}), nil, function() return "Active Reports: " .. table.Count(moderator.reports or {}) end)
    panel:AddHeader("Players: " .. #player.GetAll() .. "/" .. game.MaxPlayers(), nil, function() return "Players: " .. #player.GetAll() .. "/" .. game.MaxPlayers() end)
    ...
    panel:AddHeader("Staff Online: " .. GetOnlineStaff(), nil, function() return "Staff Online: " .. GetOnlineStaff() end)
end
