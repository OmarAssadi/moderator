...
function CATEGORY:Layout(panel)
    ...
    parent.Think = function(this)
        for k, v in pairs(moderator.groups) do
            if (not IsValid(line[k]) and moderator.lastGroup ~= k and not excluded[k]) then
    ...
    parent.OnClickLine = function(this, line, selected)
        if (not moderator.lastGroup) then return end
        local groupTable = moderator.GetGroupTable(moderator.lastGroup)
        local otherGroupTable = moderator.GetGroupTable(line.group)

        if (selected and line.group ~= groupTable.inherit) then
            if ((otherGroupTable.immunity or 0) > (groupTable.immunity or 0)) then return end
    ...
    choose.OnSelect = function(this, index, value, data)
        ...
        if (not data or value == "New...") then
            Derma_StringRequest("New Group", "Enter a group identifier for this new group.", "", function(text)
                if (#text < 1 or not text:find("%S")) then return moderator.Notify("You did not provide a valid group identifier.") end
        ...
        if (LocalPlayer():CheckGroup("owner") and not moderator.defaultGroups[data]) then
            ...
            delete.DoClick = function(this)
                Derma_Query("Are you sure you want to delete this group? It can NOT be undone!", "Delete Group", "No", function() end, "Yes", function()
        ...
        if (not found and parent.choices.none) then
    ...
    for k, v in SortedPairsByMemberValue(moderator.groups, "immunity", true) do
        if (k == "__SortedIndex") then continue end

        if (not moderator.lastGroup) then
