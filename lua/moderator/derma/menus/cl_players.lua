...
function CATEGORY:Layout(panel)
    ...
    for k, v in SortedPairsByMemberValue(moderator.commands, "name") do
        if (v.hidden or not moderator.HasPermission(k)) then continue end
...
function PANEL:Init()
    ...
    self.checked.DoClick = function(this)
        moderator.selected[self.player] = not moderator.selected[self.player]

        if (not moderator.selected[self.player]) then
