...
function d_findVariable(s, i, len, lastType)
    ...
    while true do
        ...
        elseif lastType then
            ...
            return val, i, lastType
        else --	This will occur if the very first character in the vON code is wrong.
...
_deserialize = {
    ...
    ["table"] = function(s, i, len, unnecessaryEnd)
        ...
        while true do
            ...
            elseif numeric then
                ...
                ret[ind] = val
                ind = ind + 1
            else --	Otherwise, if it's the key:value component...
                ...
                else --	Otherwise the key will be read.
    ...
    ["number"] = function(s, i, len)
        ...
        a = find(s, "[;:}~]", i)
        if a then return tonumber(sub(s, i, a - 1)), a - 1 end
    ...
    ["Entity"] = function(s, i, len)
        ...
        --	Locals, locals, locals, locals
        a = find(s, "[;:}~]", i)
        if a then return getEnt(tonumber(sub(s, i, a - 1))), a - 1 end
    ...
    ["Vector"] = function(s, i, len)
        ...
        if x and y and z then return Vector(x, y, z), a - 1 end
    ...
    ["Angle"] = function(s, i, len)
        ...
        if p and y and r then return Angle(p, y, r), a - 1 end
...
_serialize = {
    ...
    ["string"] = function(data, mustInitiate, isNumeric, isKey, isLast) return "\"" .. gsub(data, "\"", "\\\"") .. "v\"" end,
    --	Fastest.
    ...
    --	Fastest.
    ["nil"] = function(data, mustInitiate, isNumeric, isKey, isLast) return "@" end,
...
_d_meta = {
    __call = function(self, str)
        if type(str) == "string" then return _d_table(str, nil, #str, true) end
...
_s_meta = {
    __call = function(self, data)
        if type(data) == "table" then return _s_table(data, nil, nil, nil, nil, true) end
