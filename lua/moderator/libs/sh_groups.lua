...
if (SERVER) then
    ...
    net.Receive("mod_GroupUpdate", function(length, client)
        if (not client:CheckGroup("owner")) then return end
    ...
    function moderator.CreateGroup(group)
        if (not group:find("%S")) then return end
    ...
    net.Receive("mod_GroupUpdate", function()
        ...
        local typeIndex = net.ReadUInt(8)
        local value = net.ReadType(typeIndex)
        if (not moderator.groups[group]) then return end
    ...
    function moderator.UpdateGroup(group, key, value)
        if (not LocalPlayer():CheckGroup("owner")) then return end
        net.Start("mod_GroupUpdate")
        net.WriteString(group)
        net.WriteString(key)
        net.WriteType(value)
...
function moderator.GetGroup(client)
    ...
    if (not moderator.groups[group]) then
        group = "user"
    end
...
function moderator.GetGroupTable(group, default, noDefault)
    ...
    if (not noDefault) then return default or moderator.groups.user end
end

function moderator.HasPermission(command, client, group)
    client = client or (CLIENT and LocalPlayer())
    if (not IsValid(client)) then return SERVER end
    group = group or moderator.GetGroup(client)
    local groupTable = moderator.GetGroupTable(group)
    if (type(groupTable.access) == "boolean" and groupTable.access) then return true end
    if (groupTable.access and groupTable.access[command]) then return true end

    if (groupTable.inherit and groupTable.inherit ~= group) then
        local otherGroup = moderator.GetGroupTable(groupTable.inherit)
        if (otherGroup and (otherGroup.immunity or 0) > (groupTable.immunity or 0)) then return false end
        if (otherGroup.inherit and otherGroup.inherit == group) then return false end
...
function moderator.CheckGroup(client, group)
    if (not IsValid(client) or client:GetNWString("usergroup", "user") == group) then return true end
    local ourGroupTable = moderator.GetGroupTable(moderator.GetGroup(client), nil, true)
    if (not ourGroupTable) then return false end
